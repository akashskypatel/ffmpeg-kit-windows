...
for dir in prebuilt/*/ffmpeg_git*; do
  if [[ -d $dir ]]; then # else there were none, and it passes through the string "prebuilt/*..." <sigh>
    cd "$dir"
...
rm -rf prebuilt/distros # free up space from any previous distros
if [[ $1 != "" ]]; then
  prettified_ver=v$1
fi
