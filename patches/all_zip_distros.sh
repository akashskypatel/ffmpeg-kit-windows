...
cd prebuilt/win32/ffmpeg_git
  git_version=`git rev-parse --short HEAD` # "all" of them should match this
cd ../../..
mkdir -p prebuilt/distros # -p so it doesn't warn
date=`date +%Y-%m-%d`
...
root="prebuilt/distros/$date_version"
...
do_shareds() {
  do_shared ./prebuilt/win32/ffmpeg_git_shared.installed $root/32-bit/ffmpeg-shared
  do_shared ./prebuilt/x86_64/ffmpeg_git_shared.installed $root/64-bit/ffmpeg-shared
  cd prebuilt/distros
...
do_xp_compat_and_zip() {
  copy_ffmpeg_binaries ./prebuilt/win32/ffmpeg_git_xp_compat "$root/32-bit/ffmpeg-static-xp-compatible"
  copy_ffmpeg_binaries ./prebuilt/x86_64/ffmpeg_git_xp_compat "$root/64-bit/ffmpeg-static-xp-compatible"
  cd prebuilt/distros
...
do_statics() {
  copy_ffmpeg_binaries ./prebuilt/win32/ffmpeg_git "$root/32-bit/ffmpeg-static"  
  copy_ffmpeg_binaries ./prebuilt/x86_64/ffmpeg_git "$root/64-bit/ffmpeg-static" 
  cd prebuilt/distros
...
readme=./prebuilt/distros/readme.txt
